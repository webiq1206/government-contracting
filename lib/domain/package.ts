...
/**
 * Document kinds the package builder owns.
 *
 * It clears each slot before writing it, so anything else stored under one of
 * these names is deleted by the next build. Exported so the upload route can
 * refuse them rather than each place having to remember the rule.
 */
export const RESERVED_KINDS = Object.values(ARTIFACT_KIND);
...
/**
 * What a run that could not perform the audit is allowed to store.
 *
 * The rule is that a run which did not check anything may not improve the
 * record. It had previously kept only the deterministic `elig_*` findings and
 * recomputed readiness from what was left, so the AI findings from the last
 * successful audit were discarded by a run that had read nothing: a package
 * held back by three blockers became ready to submit because the AI key had
 * been removed. Less information arrived as better news.
 *
 * So everything survives, and the caller records that today's pass did not
 * run. A carried-over blocker dated a week ago is a prompt to go and look; a
 * silently cleared one is the product asserting compliance it never checked.
 */
export function findingsAfterSkippedAudit(
  prior: AuditFinding[] | null | undefined
): AuditFinding[] {
  return [...(prior ?? [])];
}

/** Findings the AI pass produced, as opposed to the deterministic ones. */
export function aiAuditFindings(findings: AuditFinding[] | null | undefined): AuditFinding[] {
  return (findings ?? []).filter((f) => !String(f.id ?? "").startsWith("elig_"));
}

export type AuditRunState = "pending" | "clean" | "issues" | "skipped" | "never";
...
/**
 * What the submission page says about the independent audit.
 *
 * Pure, because the interesting cases are combinations that are awkward to
 * reach in a browser: an audit that has never run, one that ran and was later
 * skipped, and one skipped while carrying blockers forward. Each has to read
 * as a different sentence, and the last one is the one that used to be a grey
 * aside under a package marked ready.
 */
export function auditNotice(input: {
  status: string | null | undefined;
  /** When the AI pass last completed. Null means never. */
  ranAt: Date | string | null | undefined;
  findings: AuditFinding[] | null | undefined;
  /** Formats a date for this operator. Injected so this stays pure. */
  formatDate: (d: Date) => string;
}): AuditNotice | null {
  const { status, findings, formatDate } = input;
  const ran =
    input.ranAt instanceof Date
      ? Number.isNaN(input.ranAt.getTime())
        ? null
        : input.ranAt
      : typeof input.ranAt === "string" && input.ranAt.length > 0
        ? (() => {
            const d = new Date(input.ranAt as string);
  ...
  if (status === "pending") {
    return {
      tone: "info",
      headline: "Compliance audit is running",
      detail:
        "It re-reads the solicitation against this package and flags anything missing or non-compliant.",
    };
  }

  if (status === "skipped") {
    const carried = aiAuditFindings(findings);
    const blockers = openAuditBlockers(carried).length;
    if (ran == null) {
      // Never completed, so there is nothing to carry and nothing to date.
      return {
        tone: "warn",
        headline: "The compliance audit has never run on this package",
        detail:
          "Either the solicitation text could not be read or the AI key is not set. The eligibility checks above did run and still apply. Read the package against the solicitation yourself before submitting.",
      };
    }
    const when = formatDate(ran);
    if (blockers > 0) {
      return {
        tone: "warn",
        headline: `Today's compliance audit could not run. ${blockers} blocker${
        blockers === 1 ? " from" : "s from"
      } ${when} still ${blockers === 1 ? "stands" : "stand"}`,
        detail:
          "The findings below are from that run, not from today, so they may not reflect the package as it is now. They are kept rather than cleared: a run that read nothing cannot clear a blocker.",
      };
    }
    return {
      tone: "warn",
      headline: `Today's compliance audit could not run. Showing the audit from ${when}`,
      detail:
        "Anything changed since then has not been audited. The eligibility checks above did run and still apply.",
    };
  }

  if (status === "clean" && openAuditBlockers(findings).length === 0) {
    return {
      tone: "info",
      headline: ran ? `Compliance audit found no issues on ${formatDate(ran)}` : "Compliance audit found no issues",
      detail: "Still worth a final human cross-check against the solicitation.",
    };
  }

  if (status == null && ran == null) {
    return {
      tone: "warn",
      headline: "No compliance audit yet",
      detail:
        "Nothing has re-read this package against the solicitation. The eligibility checks above are all that has run.",
    };
  }
...
/**
 * The four things that decide whether a package can go, and the one number
 * that summarises them.
 *
 * The number and the list have to be computed together or they drift. They had
 * drifted: the headline was satisfied_count/total_mandatory, which is one of
 * the four rows, so a package with its forms in order rendered "100%" in
 * display type above a checklist with three items unticked and a submit button
 * that did not work. A confident figure that answers a narrower question than
 * it appears to is worse than no figure, because nobody reads past it.
 */
export function packageChecklist(input: {
  bidAmount: number | null;
  marginText: string;
  bidText: string;
  ...
  findings: AuditFinding[] | null | undefined;
  ready: boolean;
  /** Deterministic blockers plus open audit blockers. */
  outstanding: number;
  auditStatus: string | null | undefined;
}): { rows: ChecklistRow[]; percent: number } {
  const { validation, ready, outstanding } = input;
  const openBlockers = openAuditBlockers(input.findings).length;
  const rows: ChecklistRow[] = [
    {
      title: "Pricing rollup",
      detail:
        input.bidAmount != null
          ? `Bid ${input.bidText} \u00b7 margin ${input.marginText}`
          : "Margin and quote checks",
      ok: input.bidAmount != null && input.bidAmount > 0,
    },
    {
    ...
    },
    {
      ...
      detail: openBlockers === 0 ? "No open blocker findings" : "Open audit blockers remain",
      ok: openBlockers === 0,
    },
    {
      title: "Compliance review",
      /*
       * "In progress" was the fallback for every not-ready package with no
       * deterministic blocker, including the ones where nothing was running at
       * all: a skipped audit left an open finding gating submission and a row
       * claiming work was under way. It asserted activity in order to avoid an
       * empty cell, which is the failure this product treats as the serious
       * one. It now says what is left, or that nothing has checked.
       */
      detail: ready
        ? "No open exceptions"
        : outstanding > 0
          ? `${outstanding} item${outstanding === 1 ? "" : "s"} to finish`
          : input.auditStatus === "pending"
            ? "Audit running"
  ...
  return {
    rows,
    percent: Math.round((rows.filter((r) => r.ok).length / rows.length) * 100),
  };
}
