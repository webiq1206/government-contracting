...
export async function readPipelinePulse(): Promise<PulseFinding[]> {
  ...
  const [runs, open, samKeyPresent, claudeKeyPresent, samError, quota, connection, heartbeat, stuck, paused, activeOrgCount, aiTrouble] = await Promise.all([
    ...
    // Liveness straight from the worker process. A failed read propagates to
    // Today, which shows the pulse as unavailable. Treating an unreadable
    // heartbeat as an absent one can make a platform outage look healthy.
    readWorkerHeartbeat(),
