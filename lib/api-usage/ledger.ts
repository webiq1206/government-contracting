...
export async function beginUsage(
  ...
  await transaction(async (client) => {
    ...
    const price = await client.query(
      "select max_request_cost::text from api_usage_rates where provider=$1 and service=$2",
      [provider, service],
    );
    ...
    for (const limit of limits.rows) {
      ...
      if (limit.monthly_limit != null && identity.source !== "tenant") {
        ...
        const totals = await client.query(
          `select coalesce(sum(coalesce(provider_cost,reserved_cost)),0)+$4::numeric > $5::numeric as exceeds,
          count(*) filter(where provider_cost is null and reserved_cost=0) as unknown from api_usage_events
    ...
    await client.query(
      ...
      (id,org_id,user_id,provider,service,feature,workflow,related_id,credential_source,credential_fingerprint,billing_accepted,billing_status,reserved_cost)
...
export async function finishUsage(
  ...
  await query(
    ...
    estimated_cost=(select sum((u.value::text)::numeric * (r.rates->>u.key)::numeric / 1000000)
      from api_usage_rates r,jsonb_each($4::jsonb) u where r.provider=e.provider and r.service=e.service),
