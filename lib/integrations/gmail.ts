...
export const gmail = {
  ...
  async fetchReplies(
  ...
  ): Promise<{ disabled?: boolean; replies: { threadId: string; from: string; snippet: string; messageId: string }[] }> {
    const client = await gmailClient();
    ...
    try {
      ...
      const replies: { threadId: string; from: string; snippet: string; messageId: string }[] = [];
