      ...
      */}
      ...
      {state === "failed" && onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="coarse:min-h-11 underline underline-offset-2 hover:text-foreground"
        >
          Try again
        </button>
      )}
  ...
  return (
    <div className="rounded-md border border-gold/40 bg-gold/10 px-3 py-2 text-xs">
      ...
      <div className="mt-2 flex flex-wrap gap-3">
        <button type="button" onClick={onUse} className="btn-ghost coarse:min-h-11 px-3 text-xs">
          Put it back
        </button>
        <button
          type="button"
          onClick={onDiscard}
          className="coarse:min-h-11 text-muted-foreground underline underline-offset-2 hover:text-foreground"
        >
          Discard it
        </button>
      </div>
