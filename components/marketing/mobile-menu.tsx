      ...
      {open &&
        createPortal(
          <dialog
            ...
            className="bco-mobile-dialog fixed inset-0 m-0 h-full max-h-none w-full max-w-none border-0 bg-ink/50 p-3 text-foreground backdrop:bg-black/30"
          >
            <div className="ml-auto flex max-h-[calc(100dvh-1.5rem)] w-full max-w-sm flex-col overflow-y-auto rounded-xl border border-border bg-surface p-4 shadow-xl">
              ...
              >
                {[
                  ...
                  { href: "/about", label: "About BrostCo" },
                ].map(({ href, label }) => (
                  <a
                    key={href}
                    href={href}
                ...
                <a
                  href={loginHref}
                  className="mt-2 flex min-h-11 items-center rounded-md border-t border-border px-3 text-base"
                >
                  Log in
                </a>
                <a href={signupHref} className="btn-primary mt-2 min-h-11">
                  Start free trial
                </a>
              </nav>
            </div>
