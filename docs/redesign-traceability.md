...
| A run's decision line on a subcontractor | Attached to the opportunity only | Attached to both | Timeline | Timeline | The runner writes the line carrying an agent's summary and reasoning, and it passed only `opportunityId` although the payload holds both. `sub-verify` wrote "Seed 001 Electric ruled out: located in NY, work is in TX" against no subcontractor at all, and `subActivityLogs` selects on `subcontractor_id`. The one place somebody goes to ask what happened to a subcontractor could not show the sentence that answers it. Verified by running the real agent and reading the record: two entries where there had been none |
| Why this counts as an oversight | - | - | - | - | The org-mismatch log four lines above it in the same file already tagged both, and the column has existed throughout |
| A future log losing a reference | Possible | A failing test | - | - | `tests/agent-log-refs.test.ts` pins `recordRefs`, including that a missing id is null rather than undefined: undefined would be dropped from the parameter list and shift every column after it |
| An upload claiming a generated document's slot | Stored, then deleted by the next package build | Refused, with the reason | Message | Message | `storeDoc` clears a slot before writing it, which is right for a regenerated artifact and destructive for anything else. The upload route takes `kind` from the request, so the only thing between an operator's signed bid bond and that delete was the interface happening never to send a colliding value. Verified through the real route: `cover_letter` refused with a 400, `requirement_document` accepted |
| `Cover_Letter` and other near-misses | - | Allowed, deliberately | - | - | The builder deletes on an exact match, so a differently-cased name is a different slot it never touches. The check compares the sanitized value, which is what actually reaches the database |
| The reserved list | - | Derived from `ARTIFACT_KIND` | - | - | A new generated artifact is reserved the day it is added rather than the day somebody remembers the guard. `tests/reserved-document-kinds.test.ts` also pins the delete the reservation exists for, so if that goes the restriction gets reconsidered instead of outliving its reason |
| A subcontractor document whose new status could not be written | Swallowed, and still reported as newly lapsed | Reported as unwritten, and not counted as changed | Log, summary | Log, summary | The update ended in `.catch(() => {})` and the document was pushed onto the lapsed list regardless, so a failed write sent a digest saying a subcontractor was LAPSED while the row still said active. That row is what everything else reads: no work goes out behind a lapsed certificate is enforced off the status, so an uninsured subcontractor stayed cleared for work and nothing recorded that a write had failed. Proved by making the update throw against the real database: the run now logs `status-write-failed` at error level naming the subcontractor, where before it reported "1 newly lapsed" |
| An account whose sweep threw | Skipped, while the summary said it had been swept | Named, and the run is not ok | Summary | Summary | "Compliance sweep across 5 org(s)" was printed whether five had run or two. An account whose coverage was never checked read exactly like an account with nothing expiring. Isolating one account's failure is right and worth keeping; not saying so is not |
| Whether a failed write makes the run not ok | - | No, deliberately | - | - | The run completed and did report the problem, and the `status-write-failed` log carries status `error`, which is the field `automation-status` selects on. Flagging the run as well would count one failure twice |
| Signing a device out when the delete fails | "That session had already ended", or `ok: true` | 500, naming what is still signed in | Message | Message | Both helpers swallowed the error and returned 0, and 0 does not differ from "the row was already gone". So a refused delete rendered as "0 devices signed out" in the success tone while every session stayed live. The person on that page is usually the person who thinks somebody else is in their account. Verified with a trigger making every delete on `sessions` throw: 500 with the honest message, where it had been 200 and a zero. My own code, from the account piece |
| "That session had already ended" | Covered both cases | Kept for the case where it is true | Message | Message | It is a correct statement about a session that is gone and a false one about a session still live because the write failed. Two facts, two sentences |
| What the failure notice says to do | - | That the device is still signed in, and to change the password | Message | Message | A notice that only says the action failed leaves the reader to assume the safer thing, which here is the wrong one |
| The 240 other swallowed catches | - | Left alone, deliberately | - | - | Most are genuinely best effort: a tracking pixel, a cleanup delete, a bookkeeping row. What separates a defect from a correct swallow is whether the caller then reports success or makes a decision as though the write had happened, which is a judgement about each call rather than something a scan can decide. The four fixed here were found that way and are recorded individually |
| A trial warning email that failed to send | Logged as "Warned {email}" | Logged as unsent, with the error | Log | Log | The send swallowed its error and the log claimed the warning had gone. That log row is also the dedupe marker, so the customer was never warned at that threshold again: one transient mail failure permanently lost somebody's three-day notice while the audit trail said they had been told. They would find the account locked with no warning, which this file's own header calls the thing that makes people leave |
| The action a failed warning is logged under | - | `trial-warning-3d-unsent` | - | - | The dedupe matches on `action = trial-warning-3d`, so writing that action is what suppressed the retry. A different action leaves the next sweep free to try again while the threshold is still in the past |
| An expiry flip that could not be written | "0 closed", indistinguishable from nothing to close | Named, and the run is not ok | Summary | Summary | Verified with a trigger making the update throw: `ok: false` and the reason, where before it was `ok: true` and a zero. What this does NOT do is let the customer in: the access gates read `trial_ends_at` directly, which is why the sweep is not what enforces the limit. What breaks is the stored status disagreeing with the gates, so admin views, filters and analytics all say "trial" about somebody who is already locked out |
| My first wording of that finding | "still has full access" | Corrected | - | - | Written before reading the file's header, which says the gates enforce the limit independently. The defect is real and the consequence I first gave for it was not |
| Seven error-level logs | Counted as healthy runs | Counted as failures | Health page | Health page | `automation-status` classifies each run by its `status` column, not its `level`, and `logAgent` defaults status to ok. So a failed Gmail poll, a failed per-account run of the opportunity monitor, and all four passes of the concession sweep were logged at error level and read as fine by the page an operator opens to find out whether anything is broken |
| The four concession-sweep handlers | Written, and dead | Reachable | Log | Log | Each pass sits in a try/catch that logs a `*-pass-failed` error, which is the right shape. The query inside each pass ended in `.catch(() => [])`, so a failed read became an empty result and the outer catch never fired: the sweep printed "0 nudged, 0 repaired, 0 flagged" for a database it could not read, which is the sentence it prints when everything is in order |
| The pairing going wrong again | Possible | A failing test | - | - | `tests/agent-log-status.test.ts` scans for it, because the pairing is easy to leave out and invisible at the call site. Proved to work by removing one `status` and watching it name the file and action, rather than assumed from the fact that it passes |
| `level: "warn"` | - | Left as ok | - | - | A warning is not a failed run and health is a binary. Forcing warns to error would fill the page with things that are working, which is how a health page stops being read |
| "Which accounts do I work on" | `.catch(() => [])`, in fourteen places | One helper that keeps the failure separate | Summary, log | Summary, log | Six then fell back to the founding organization, which is right for a genuinely empty list and wrong for a query that threw: one database hiccup on that statement skipped every customer, ran the agent against the founding org alone, and reported success. Verified by shadowing the table with a view that throws: five agents that returned `ok: true` now return `ok: false` with the reason and an `org-list-failed` log at error status |
| "No active contracts with subcontractors attached" | Said about a lookup that had failed | Said only when it is true | Summary | Summary | The clearest wording of the whole pattern: a positive statement of absence, standing in for a question nobody could answer |
| The eight sweeps in `maintenance.ts` | Reported over zero accounts | Let the lookup throw | Log | Log | They sit inside handlers the runner already wraps, and the runner logs at error status and marks the run failed, so a deletion is the whole fix. `activeOrgIds` keeps its founding-org fallback for the genuinely empty case, through the helper |
| The pattern coming back | Possible | A failing test | - | - | `tests/org-fanout.test.ts` covers the three outcomes and scans `lib/` for the catch. The distinction it pins is the one that made this invisible: an empty list and a failed lookup are not the same fact |
