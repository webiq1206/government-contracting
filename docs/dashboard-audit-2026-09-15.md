...
| Shared table sorting | Tablet CI intermittently lost a sort action immediately after navigation. Server-rendered controls could be used before their client handler was ready. | Keep the sort picker disabled until hydration completes; retain native URL navigation and page reset. | Responsive pagination/sorting workflow checks the destination URL and first record. |
