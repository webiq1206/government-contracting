...
| Contracts and compliance | Award and source material to obligations, documents, renewals and limits | Existing deadline and compliance semantics | Existing tabs, tables and controls retained with new shared presentation. |
...
| Contextual AI with sources | Guide, sources, requirements and existing agent outputs | Less context switching; genuine chat requires grounding, job context and permissions | Keep source access and Guide me. A general conversational copilot is not introduced by a label change. |
...
The justified additions in this branch are usability behavior, not new contracting modules: server-preloaded activity, disclosure-aware deep links, focused mobile navigation, and a connected public product gallery.
