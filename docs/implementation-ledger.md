...
| 12 | No browser alert, confirm or prompt as the final interface | Implemented, and eleven call sites were violating it | `window.confirm` cannot say what an action costs, cannot be made to belong to the record it is about, and blocks the main thread while open. `window.prompt` adds a fourth problem: it cannot validate, so the one call site using it took a reason, closed, showed a toast saying it was too short, and made the operator reopen and retype. `components/confirm-dialog.tsx` replaces both. A guard test walks `components/` and `app/` and fails on the twelfth, comments stripped so the files explaining why they do not use it are not the ones that break it |
| 12 | Dialogs trap and restore focus | Implemented | Focus goes in, and the half people forget is that it comes back: a dialog that takes focus and drops it on the body leaves a keyboard user at the top of the document, several tab stops from where they were, with nothing saying what happened. Escape closes, the backdrop cancels and never confirms, and every caller names the act rather than saying OK |
| 12 | Accessible SVG icons in the mobile shell, no text glyphs | Implemented, and one was the exact character the brief rules out | Subcontractors was `☰`, the hamburger, which means "menu" on every device an operator owns. The others were glyphs that render differently or not at all depending on the font stack: a variation-selected sun some Android stacks draw in full colour and others draw as nothing, and a grid character missing from enough fallback fonts to show as a box. The bar is on screen for the whole session, so a missing glyph is a permanently broken control |
| 12 | The five bottom tabs the brief names | Implemented, reversing an earlier deviation | An earlier version gave the fourth slot to Inbox on the argument that a subcontractor who has written and had no answer is waiting on a person right now. That argument is good and it is not this decision to make. The signal is not lost: More carries an attention indicator, Communications is the destination behind it that somebody outside the company is waiting on, and that count is what the badge shows |
| 12 | State is never communicated by colour alone | Verified on the tab bar | `aria-current`, a rule above the active tab, and a bold label: three signals, one of which is not colour |
