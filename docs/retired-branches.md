...
The history was rewritten at some point, so most of these branches share **no
common ancestor** with `main`. `git merge-base` returns nothing, and
`git diff main...branch` then reports no differences no matter what the branch
contains. That empty diff reads as "already merged" and means nothing at all.
A full two-dot compare shows over a thousand files differing, which also means
nothing: most of it is `main` having moved on. `git log --cherry-pick` is no
better here -- it reports 122 to 270 "unmerged" commits on branches that are
one feature wide, because rewritten history breaks patch-id matching.

So the evidence used here is two things that survive a rewrite:

1. **A merged pull request**, listed per branch. That proves the work landed.
2. **The branch tip still equals the head SHA that pull request merged.** That
   proves nothing was pushed on top afterwards, which a merged PR alone does
   not establish -- a branch can be merged and then carry on.

Both were re-verified against the live repository, per branch, on 2026-09-08.

## Deleting them

The credential available to the automation can push refs but not delete them:
`git push --delete` returns HTTP 403 from GitHub, with the agent proxy healthy
and reporting no relay failures, so it is a permission boundary rather than a
network fault. Deletion needs a human or a token with `delete_ref`.

```sh
# From a clone, with a credential that may delete refs:
git push origin --delete \
  audit/production-readiness-2026-09-07 \
...
Or `gh api -X DELETE repos/webiq1206/government-contracting/git/refs/heads/<branch>`
per branch, or Branches -> the bin icon in the GitHub UI.

## The branches

| Branch | Tip | Last commit | Merged as | Subject |
| --- | --- | --- | --- | --- |
| `audit/production-readiness-2026-09-07` | `1a3bd9b1b8e59c4c130ba5e60658092a0d9f5852` | 2026-09-07 | #112 | Record verified reply safeguards and remaining visual re |
...
| `claude/git-sync-check-nqy8by` | `19a4bceea79c71ea362e85ec92b2cfde93884dc7` | 2026-08-27 | #91 | Merge pull request #91 from webiq1206/claude/git-sync-ch |
...
`cursor/admin-ia-clarity-5f54` is **excluded**. Its pull request (#6) merged,
but the branch then took one more commit, `3bf9608 Rewrite How it works with
every current pipeline step`, which is not in `main` by any check available
here. The same work appears to have reached `main` through #7 on
`cursor/how-it-works-steps-5f54`, but the files have changed enough since
August that the claim cannot be settled by comparing them, and "appears to"
is not the standard for deleting the only copy of something. It costs nothing
to leave one branch alone.

`claude/three-pane-workspace-review-bazs4s` is also excluded: it is the active
working branch, not a leftover.
